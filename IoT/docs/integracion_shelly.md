...
- Shelly Cloud controla el canal configurado del Shelly Pro 4PM.
...
Agregar a `api/config.local.php`:
...
```text
https://DOMINIO/ID-Industrial/api/shelly_webhook.php?token=TOKEN&device_id=34987a67da6c&channel=0&output=1
https://DOMINIO/ID-Industrial/api/shelly_webhook.php?token=TOKEN&device_id=34987a67da6c&channel=0&output=0
```
...
2. En `Administrar dispositivos`, pulsar `Probar` y confirmar estado `ONLINE`.
3. Confirmar que voltaje, potencia y salida correspondan al canal esperado.
4. Pulsar `Encender` y luego `Apagar`.
5. Conectar la sirena mediante un instalador electrico y repetir la prueba.
6. Provocar una alarma de sensor controlada y verificar encendido automatico.
7. Silenciar desde web y app; verificar que Shelly se apague y el ESP32 conserve la revision pendiente.
