...
constexpr uint8_t ESTACION_MANUAL_LECTURAS_CONFIRMAR = 2;
constexpr uint8_t ESTACION_MANUAL_LECTURAS_LIMPIAR = 2;
...
uint8_t estacionManualLecturasActivas = 0;
uint8_t estacionManualLecturasLimpias = 0;
...
bool estacionManualAnterior = false;
...
void leerSensores() {
  ...
  lecturaEstacionManual = digitalRead(PIN_ESTACION_MANUAL);
...
void estabilizarDetectores() {
  ...
  bool estacionRawActivada = lecturaEstacionManual == LOW;
  if (estacionRawActivada) {
    if (estacionManualLecturasActivas < 20) {
      estacionManualLecturasActivas++;
    }
    estacionManualLecturasLimpias = 0;
  } else {
    if (estacionManualLecturasLimpias < 20) {
      estacionManualLecturasLimpias++;
    }
    estacionManualLecturasActivas = 0;
  }

  if (estacionManualLecturasActivas >= ESTACION_MANUAL_LECTURAS_CONFIRMAR) {
    estacionManualActivada = true;
  }
  if (estacionManualLecturasLimpias >= ESTACION_MANUAL_LECTURAS_LIMPIAR) {
    estacionManualActivada = false;
  }
}
...
bool hayEstacionManualActivada() {
  return estacionManualActivada;
}

void actualizarAlarmas() {
  bool hayFlama = hayFlamaDetectada();
  bool hayGas = hayGasDetectado();
  bool hayEstacionManual = hayEstacionManualActivada();
  ...
  bool nuevaCondicionCritica =
    ...
    || (hayEstacionManual && !estacionManualAnterior)
    || (temperaturaPeligrosa && !temperaturaPeligrosaAnterior);

  String estadoAnterior = estado;
  peligroActivoAhora = hayFlama || hayGas || hayEstacionManual || temperaturaPeligrosa;
  ...
  estacionManualAnterior = hayEstacionManual;
...
bool enviarDatosAPI() {
  ...
  int estacionManualBinario = hayEstacionManualActivada() ? 1 : 0;
  ...
  json += "\"estacion_manual_activada\":" + String(estacionManualBinario) + ",";
...
String tipoAlertaActual() {
  ...
  bool estacionManualActual = hayEstacionManualActivada();

  if (estacionManualActual && gasActual && flamaActual) {
    return "Estacion manual + Flama + Humo/Gas";
  }

  if (estacionManualActual) {
    return "Estacion manual";
  }
...
void imprimirMonitorSerie() {
  ...
  Serial.print("Estacion manual: ");
  Serial.println(hayEstacionManualActivada() ? "ACTIVADA" : "NORMAL");
...
<main>
  ...
  <section class="panel grid">
    ...
    <div class="dato" id="estacionManualCard">
      <span>Estacion manual</span>
      <b id="estacionManual">--</b>
      <small id="estacionManualEstado">--</small>
    </div>
  </section>
...
async function actualizar() {
  try {
    ...
    document.getElementById('estacionManual').textContent =
      datos.estacionManualActivada ? 'ACTIVADA' : 'NORMAL';
    ...
    actualizarDetector('estacionManualCard', 'estacionManualEstado', Boolean(datos.estacionManualActivada));
...
void datosJSON() {
  ...
  json += "\"estacionManualActivada\":" + String(hayEstacionManualActivada() ? "true" : "false") + ",";
